...
create index if not exists idx_expenses_month_year  on expenses(
...
create index if not exists idx_revenues_month_year on revenues(
  extract(year  from revenue_date)::int,
  extract(month from revenue_date)::int
);
